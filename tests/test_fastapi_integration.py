...
def test_search_success(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: [{"title": "T", "doi": "10.1/test", "keywords": ["a"]}],
    )
...
def test_search_service_failure(monkeypatch):
    monkeypatch.setattr(
        "services.openalex.search_works",
        lambda **kwargs: (_ for _ in ()).throw(APIRequestError("down")),
    )
    r = client.get("/search", params={"keyword": "test"})
    assert r.status_code == 400
...
def test_paper_details_success(monkeypatch):
    monkeypatch.setattr(
        "services.openalex.get_work", lambda identifier: {"title": "Paper", "doi": identifier}
    )
...
def test_paper_details_not_found(monkeypatch):
    monkeypatch.setattr(
        "services.openalex.get_work", lambda identifier: (_ for _ in ()).throw(NotFoundError("no"))
    )
...
def test_author_and_journal_endpoints(monkeypatch):
    monkeypatch.setattr(
        "services.openalex.search_authors", lambda name, per_page=10: [{"name": "A"}]
    )
    monkeypatch.setattr(
        "services.openalex.search_sources", lambda name, per_page=10: [{"name": "J"}]
    )
...
def test_citations_and_related(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda doi, fields=None: {"title": "T", "citationCount": 1, "referenceCount": 0},
    )
    monkeypatch.setattr(
        "services.semanticscholar.get_citations",
        lambda doi, limit=20: [{"title": "Cite", "year": 2020}],
    )
    monkeypatch.setattr(
        "services.semanticscholar.get_references",
        lambda doi, limit=20: [{"title": "Ref", "year": 2019}],
    )
    monkeypatch.setattr(
        "services.semanticscholar.get_recommendations",
        lambda doi, limit=10: [{"title": "Rel", "year": 2018, "externalIds": {"DOI": "10.1/r"}}],
    )
...
def test_export_and_summarize_and_pdf(monkeypatch):
    ...
    monkeypatch.setattr(
        "services.semanticscholar.get_paper_by_doi",
        lambda doi, fields=None: {"title": "T", "abstract": "A. B. C.", "tldr": None},
    )
    monkeypatch.setattr(
        "services.openalex.get_work", lambda doi: {"abstract": "An abstract. Sent two."}
    )
    ...
    # pdf
    monkeypatch.setattr(
        "services.openalex.get_work", lambda doi: {"open_access_pdf": "https://example.com/p.pdf"}
    )
